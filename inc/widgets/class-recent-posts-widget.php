...
if( !class_exists( 'DIERREWEB_Recent_Posts' ) ) {
  Class DIERREWEB_Recent_Posts extends WPH_Widget {

    function __construct() {

      $widget_ops = array(
        'classname'   => 'widget_recent_posts',
        'description' => esc_html__( 'The most recent posts on your site', 'dr' ) );

        parent::__construct( 'recent_posts', esc_html__( 'DIERREWEB Recent Posts', 'dr' ), $widget_ops );
    ...
    function widget( $args, $instance ) {
      ...
      $cache = wp_cache_get( 'dierreweb_recent_posts', 'widget' );
      ...
      if( $posts->have_posts() ) : ?>
            ...
            <li>
              <?php if( $thumb ) : ?>
                <?php if( has_post_thumbnail() ) : ?>
                  <a class="recent-post-thumbnail" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
              ...
              <div class="recent-post-info">
                ...
                <?php if( $show_date ) : ?>
                  <a href="<?php echo esc_url( get_month_link( '', '' ) ); ?>">
                    <?php wp_kses_post( the_date( '', '<time class="post-date" datetime="' . get_the_date( 'c' ) . '">', '</time>' ) ); ?>
                  </a>
                <?php endif ?>

                <?php if( $show_comments ) : ?>
                  <a class="recent-post-comment" href="<?php comments_link(); ?>">
        ...
        wp_cache_set( 'dierreweb_recent_posts', $cache, 'widget' );
    ...
    function flush_widget_cache() {
      wp_cache_delete( 'dierreweb_recent_posts', 'widget' );
    }

    function form( $instance ) {

      $defaults = array(
 				'title'         => esc_attr__( 'Recent Posts', 'dr' ),
 				'limit'         => 5,
 				'offset'        => 0,
 				'order'         => 'DESC',
 				'orderby'       => 'date',
